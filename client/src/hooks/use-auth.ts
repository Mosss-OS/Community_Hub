...
// Auth token is now handled via httpOnly cookies, no client-side storage needed
export function setAuthToken(token: string | null) {
  // Token is handled via httpOnly cookies on the server
  // This function is kept for compatibility but does nothing
}

export function getAuthHeader(): string | null {
  // Auth is handled via cookies, not headers
  return null;
}
