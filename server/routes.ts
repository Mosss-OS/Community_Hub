  ...
  app.post("/api/auth/signup", signupLimiter, csrfProtection, async (req, res) => {
   ...
   app.post("/api/auth/2fa/setup", isAuthenticated, async (req: AuthenticatedRequest, res) => {
     try {
       ...
       // Store in DB with enabled=false
       try {
         await storage.enable2FA(req.user!.id, secret.base32, backupCodes);
         // Then disable it until verified
         await storage.disable2FA(req.user!.id);
       } catch (e) {
   ...
   app.post("/api/auth/2fa/verify", isAuthenticated, async (req: AuthenticatedRequest, res) => {
     try {
       ...
       // Get 2FA settings from DB
       const settings = await storage.get2FASettings(req.user!.id);
       if (!settings) return res.status(400).json({ message: "Setup required first" });
       ...
       // Enable 2FA
       await storage.enable2FA(req.user!.id, settings.secret, settings.backupCodes || []);
   ...
   app.get("/api/auth/2fa/status", isAuthenticated, async (req: AuthenticatedRequest, res) => {
     try {
       const settings = await storage.get2FASettings(req.user!.id);
       res.json({
...
Prayer: Thank You, Lord, for Your amazing grace and mercy. Help me to extend the same to others. In Jesus' name, amen.`,
  ...
  // Seed database removed - use separate script
  // await seedDatabase();
...
// Seed database code removed - use separate script if needed
